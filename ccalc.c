...
printResults(long long result)
{
  ...
  // decimal numbersfprintf(stderr, "0x%llx ", result);
  // use fprintf to print SUM_POSITIVE and SUM_NEGATIVE finish
  // printing the line note space separate the values and put a new
  // line at the end.
...
writeResults(long long result) {
  ...
  long long mem_leng = (long long)(&CALC_DATA_END) - (long long)(&CALC_DATA_BEGIN);
  if (write(1, &CALC_DATA_BEGIN, mem_leng) != mem_leng) {
    fprintf(stderr, "ERROR: write results failed\n");
  }
}
